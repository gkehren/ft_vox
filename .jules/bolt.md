...
## 2024-05-25 - [Boost.Asio Network Receive Zero-Allocation Buffer Pattern]
**Learning:** [In a UDP `async_receive_from` callback loop, creating a new `std::vector` inside the handler to process the received buffer incurs a heap allocation for every single incoming packet, which degrades performance in a hot networked game loop. Because `handleMessage` processes synchronously or copies what it needs, the buffer's lifetime does not need to extend beyond the function scope.]
**Action:** [Use `thread_local std::vector<uint8_t>` combined with `.assign()` inside the `handleReceive` callback. This guarantees thread-safety (Boost.Asio doesn't re-enter on the same thread simultaneously) while reusing the vector's capacity, resulting in zero dynamic allocations on the hot path after the initial packet reception.]
