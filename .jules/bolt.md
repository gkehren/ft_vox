...
## 2024-05-18 - Avoid Dynamic Memory Allocations on High-Throughput Network Receive Loops
**Learning:** In the `ft_vox` networking architecture, the high-throughput receive loops (`Client::handleReceive` and `Server::handleReceive`) process individual packets rapidly. Previously, these loops allocated a locally scoped `std::vector<uint8_t>` for each packet, leading to frequent dynamic memory allocations and deallocations on the heap. While standard downstream processing (like `ByteBuffer` instantiation) still inherently requires data copying, the allocations prior to instantiation can be minimized.
**Action:** When working on hot paths or loops handling continuous incoming data (like UDP receive loops), utilize a `thread_local std::vector` and reuse its capacity via `.assign()` or `.clear()`. This approach eliminates the overhead of repeated heap allocations without introducing data races in the local thread scope, serving as a safe and effective micro-optimization in C++ network programming.
