...
void main()
{
    ...
    skyColor += vec3(0.48, 0.14, 0.04) * sunset * horizonBand * sunsetFacing;
    ...
    // 5. Draw Sun disc — warmer / brighter so the sky feels alive
    float sunGlow = max(dot(V, sunDir), 0.0);
    float sunDisc = smoothstep(0.9994, 0.9997, sunGlow);
    vec3 sunColor = vec3(1.0, 0.96, 0.72) * 2.6;
    vec3 sunGlowColor = vec3(1.0, 0.55, 0.18) * pow(sunGlow, 140.0) * 0.72;
    ...
    if (abs(V.y) > 0.001) {
        ...
        if (maxDist > 0.0001) {
            ...
            if (hit) {
                ...
                vec3 nightCloud = vec3(0.08, 0.10, 0.18);
